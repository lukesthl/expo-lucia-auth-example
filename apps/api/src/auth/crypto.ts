...
export const createHash = async (data: Data, algorithm: Algorithm): Promise<string | null> => {
  ...
  if (data instanceof ReadableStream) {
    ...
    await reader?.read().then(async (chuck) => {
      const value = await createHash((chuck.value as string) || "", algorithm);
      body += value;
    });
    return body;
  }
  ...
  if (crypto?.subtle) {
    ...
    const hash = Array.prototype.map
      // @ts-expect-error - TS doesn't know about Uint8Array
      .call(new Uint8Array(buffer), (x) => ("00" + (x as string).toString(16)).slice(-2))
      .join("");
    return hash;
  }
  return null;
